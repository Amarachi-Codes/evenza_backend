...
export class EventServiceimpl implements EventService {
  async getEventsBookedByUser(userId: number): Promise<EventResponseDto[]> {
    try {
      logger.info(`Fetching events booked by userID: ${userId}`);

      const eventsByUser = await db.event.findMany({
        where: {
          bookings: {
            some: {
              userId,
      ...
      return eventsByUser as EventResponseDto[];
    } catch (error) {
      logger.error(`Failed to fetch events for User ID: ${userId}:`, error);
      throw new Error("Unable to retrieve booked events.");
    }
  }
  ...
  async bookEvent(data: BookingDto): Promise<BookingResponseDto> {
    ...
    if (existingBooking) {
      logger.warn(
        `Double booking attempt detected for user id ${userId} and event id ${eventId}`
      );
